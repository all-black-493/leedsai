...
import { z } from 'zod'
import { createAutomation, deleteKeyword, saveKeyword, saveListener, savePosts, saveTrigger, updateAutomationName } from "@/actions/automations"
...
export const useCreateAutomation = () => {
    const { mutate, isPending } = useMutationData(
        ['create-automation'],
        (params: { name: string, keywords?: string[] }) => createAutomation(params),
        'user-automations'
    )
...
export const useKeywords = (id: string) => {
    ...
    const { mutate: deleteKeywordMutation } = useMutationData(
        ['delete-keyword'],
        (data: { id: string }) => deleteKeyword(data.id),
        'automation-info'
    )

    return { keyword, onValueChange, onKeyPress, deleteKeywordMutation }
